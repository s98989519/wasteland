class CombatSystem {
    ...
    selectTarget(enemyId) {
        ...
        const currentAction = this.playerQueuedActions[enemyId];
    ...
    async executeTurn() {
        ...
        for (const enemy of aliveEnemies) {
            const action = this.playerQueuedActions[enemy.id];
            if (action) {
    ...
    async executePlayerAction(type, enemy) {
        return new Promise(resolve => {
            if (enemy.isDead) return resolve();
            ...
            if (type === 'attack') {
                ...
                this.prepStacks += 1;
                Logger.log(`你在對 ${enemy.name} 的空檔屏氣凝神，獲得 1 層準備。`, "positive");
            } else if (type === 'skill') {
                const skill = this.getWeaponSkill();
                ...
                Logger.log(`你擺出防禦姿態，準備硬扛 ${enemy.name} 的攻擊。`);
            } else if (type === 'dodge') {
                Logger.log(`你集中精神，試圖閃躲 ${enemy.name} 的攻擊。`);
            }
    ...
    async executeEnemyAction(enemy) {
        return new Promise(resolve => {
            ...
            if (actionType === 'stunned') {
                Logger.log(`${enemy.name} 因為猛烈衝撞而暈眩，無法行動！`);
                enemy.state = 'normal';
                setTimeout(resolve, 200);
                return;
                ...
                this.enemies.forEach(e => {
                    if (!e.isDead) {
                        ...
                        if (healAmount > 0 && this.game.ui.showFloatingText) {
                            // Slight delay so it doesn't overlap perfectly with "治療孢子" text
                            setTimeout(() => {
            ...
            let finalDmg = Math.max(0, rawDmg - totalDefBonus);
            let actionText = actionType === 'charge' ? `發起了猛烈的衝撞` : `發起了攻擊`;

            const playerAction = this.playerQueuedActions[enemy.id];
            const defType = playerAction ? playerAction.type : null;
            ...
            if (defType === "block") {
                ...
                if (roll <= dodgeChance) {
                    ...
                    dodgeAmount = Math.min(finalDmg, 2);
                    finalDmg = Math.max(0, finalDmg - 2);
                    dodgeStr = " <span style='color:var(--accent-green);'>(稍微閃躲)</span>";
                }
            }
            ...
            let msg = `${enemy.name} 對你${actionText}！`;
            
            let reductions = [];
            if (totalDefBonus > 0) reductions.push(`${totalDefBonus} <span style="color:gray;">防禦力</span>`);
            if (blockAmount > 0) reductions.push(`${blockAmount} <span style="color:gray;">防禦動作</span>`);
            if (dodgeAmount > 0) reductions.push(`${dodgeAmount} <span style="color:gray;">閃避</span>`);
            if (armorBlocked > 0) reductions.push(`${armorBlocked} <span style="color:var(--accent-orange);">護甲</span>`);
            
            if (reductions.length > 0) {
                msg += ` 你受到了 ${actualHpDamage} ( ${rawDmg} - ${reductions.join(' - ')} ) 點傷害。${dodgeStr}`;
            } else {
                msg += ` 你受到了 ${actualHpDamage} 點傷害。`;
            }
    ...
    winCombat() {
        ...
        if (aggregatedDrops['red_cap']) {
            this.game.inventorySystem.addItem('red_cap', '紅色菇傘', 'consumable', { attackBonus: 1 }, aggregatedDrops['red_cap']); // Temporarily just +1 ATK equipment or consumable
    ...
    endCombat() {
        ...
        // Cap prep stacks to 3 at the end of combat
